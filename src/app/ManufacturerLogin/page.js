...
export default function ManufacturerLoginPage() {
    ...
    const onLogin = async () => {
        try {
            setLoading(true);
            const response = await axios.post("/api/users/ManufacturerLogin", user);
    ...
    return (
        <div>
            <div className="text-center text-3xl mt-10">
                <h1>{loading ? "Processing" : "Manufacturer Login"}</h1>
            </div>
            ...
            <div className="flex flex-col items-center justify-center min-h-screen py-2 login">
                ...
                />
                <Link href='/ManufacturerForgotPassword' className="mb-3">ForgotPassword?
                </Link>
                <button
                    onClick={onLogin}
